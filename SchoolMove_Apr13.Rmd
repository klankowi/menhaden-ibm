...
\newpage
## Initialize fish
|        We will begin by initializing 1000 individual fish. These fish will have their own identities, and their positions relative to each other will be preserved for every time step. Many of the initialized parameters are based on values pulled from the literature or assumptions.    
...
setwd(paste0("G:/My Drive/Documents_Backup/Modeling/Modeling_RCode/",
             "GIFs/sch_mov/subset_6"))

timecells <- seq(1, 14012, by=30)
...
gifski(files, "sch_mov_6.gif", loop = FALSE, delay = 0.01)
```